...
kill(int pid)
{
  ...
  for(p = proc; p < &proc[NPROC]; p++){
    ...
    if(p->pid == pid){
      /*for (int i = 0; i < NPROC; i++){
        if (p->is_thread == 0 && p->mythread[i]){
          p->mythread[i]->killed = 1;
          check++;
        }
      }*/
...
clone(void (*fcn)(void*, void*), void *arg1, void *arg2, void *stack)
{
  ...
  if(mappages(th->pagetable, th->trapframe_va, PGSIZE, (uint64)(th->trapframe), PTE_R | PTE_W) < 0)
    return -1;
    ...
    for(th = proc; th < &proc[NPROC]; th++){
      if(th->parent == p && th->is_thread){
        ...
        if(th->state == ZOMBIE){
          // Found one.
          pid = th->pid;
          if(!stack || copyout(p->pagetable, (uint64)stack, (char *)&th->ustack, sizeof(th->ustack)) < 0) {
            freeproc(th);
            release(&th->lock);
            release(&wait_lock);
            return -1;
