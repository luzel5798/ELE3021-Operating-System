...
// 0 - MLFQ (Multi Level Feedback Queue)
...
void FCFS_scheduler(struct proc *p, struct cpu *c);
void MLFQ_scheduler(struct proc *p, struct cpu *c);
void priorityBoosting(void);
void deleteElt(struct queue *q);
...
//Scheduler -> L0, L1 - default RR / L2 - priority 
void
...
saveQueue()
{
  ...
  struct proc *p;
  for (int i=0; i<3; i++){
    //L[i].capacity는 현재 RUNNING state의 process까지 포함한 결과이므로.. capacity-1까지만 loop
